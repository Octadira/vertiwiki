...
### **v0.2.4** (Current Active Release — August 2026) :badge[Latest]{type=success}
* 🤖 **Semantic No-JS Fallback & AI Discoverability**: Pre-seeded semantic HTML shell and `<noscript>` navigation inside `#verti-app` for HTTP scrapers and AI agents.
* 🌐 **AI & LLM Discovery Meta Tags**: Added `<link rel="alternate">` and `<link rel="help">` metadata pointing to `llms.txt` and raw Markdown files.
* 📦 **Synchronized Distribution**: Automated build synchronization across `dist/` and `website/`.
